...
ABColourV2_ind_vars = IndependentVariableSet()

ABColourV2_ind_vars.add_variable("lag", int, [1,2,3,4,5,6,7,8])
...
>> ABColourV2_ind_vars.add_variable("color", str)
>> ABColourV2_ind_vars.add_variable("size", float)
>> ABColourV2_ind_vars.add_variable("active", bool)
>> ABColourV2_ind_vars.add_variable("count", int)

Finally, we add values to each variable. This can be done one at a time, as in the colors example below:

>> ABColourV2_ind_vars['color'].add_value("blue")
>> ABColourV2_ind_vars['color'].add_value("blue")
>> ABColourV2_ind_vars['color'].add_value("blue")

Or altogether in a comma-separated set, as in the 'count' example:

>> ABColourV2_ind_vars['count'].add_values(1,2,3,4,5)
...
>> ABColourV2_ind_vars['size'].add_value(1.0, 2)
>> ABColourV2_ind_vars['size'].add_value(2.0, 2)
>> ABColourV2_ind_vars['size'].add_value(5.0)
...
>> ABColourV2_ind_vars['active'].add_values((True, 5), False)

which is equivalent to:

>> ABColourV2_ind_vars['active'].add_value(True, 5)
>> ABColourV2_ind_vars['active'].add_value(False)
...
>> ABColourV2_ind_vars['size'].enabled = False
...
>> ABColourV2_ind_vars['size'][1.0].enabled = False
